...
\begin{document}
	...
	\begin{frame}[plain]
		...
		\begin{itemize}
			...
			\item[\color{oxfordblue}$\blacktriangleright$] It is able to construct \textbf{isoparametric meshes} representation, which conform to the geometry.
	...
	\begin{frame}[plain]
		...
		\begin{itemize}
			\item[\color{oxfordblue}$\blacktriangleright$] Wide range of finite elements available, including and not limited to hierarchical $H^1$ elements, $H(div)$ Raviart-Thomas and Brezzi-Douglas-Marini elements, and $H(curl)$ Nédélec elements.
	...
	\begin{frame}[plain]
		...
		$\newline$
		\ngshead
		\lstinputlisting[language=Python, firstline=79, lastline=95]{../examples/ngs/precond.py.rst}
	\end{frame}
	\begin{frame}[plain]
	...
	\begin{frame}
		...
		$\newline$
		\begin{itemize}
			\item [$\color{oxfordblue}\blacktriangleright$] We exposed PETSc solver in NGSolve. 
			\item [$\color{oxfordblue}\blacktriangleright$] PETSc preconditioners can be used as building blocks in NGSolve.
			\item [$\color{oxfordblue}\blacktriangleright$] Thanks to \textbf{PETSc SNES} we have a robust non-linear solver in NGSolve.
			\item [$\color{oxfordblue}\blacktriangleright$] Thanks to \textbf{SLEPc EPS} we have a robust eigenvalue solver in NGSolve.
		\end{itemize}
	\end{frame}
	\begin{frame}
		...
		\begin{itemize}
			...
			\item [$\color{oxfordblue}\blacktriangleright$] We plan to wrap also \textbf{SLEPc PEP} to solve polynomial eigenvalue problems.
		\end{itemize}
	\end{frame}
